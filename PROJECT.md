...
- 根据最新站位草图重排守城阵型：主角位于中轴 C 位；伙伴 1/2/3 位于第二排；伙伴 4 位于中轴远程后排；全场只保留一座居中防御塔，塔在城墙前、公主在城墙后。
- 新增 `BattleMapCatalog`，用 `GameplaySessionState.chapterNumber` 选择“王城前境 / 幽影密林 / 霜风隘口 / 焰龙焦土”主题；地图视觉与关卡融合，泉水、角色、塔和防线坐标仍由唯一 `BattleLayoutConfig` 管理。
- “王城前境”加入中央石板路、林地边界、地表装饰、蓝金守护塔与加宽城墙，替换大面积纯色原型底板。
- 删除旧版 4 个灰色卡槽运行路径，新增底部五人动态角色卡：`伙伴1 / 伙伴2 / 主角 / 伙伴3 / 伙伴4`，主角卡居中放大；角色卡直接读取主角与伙伴的当前人物、生命和复活状态。
- 保留左侧虚拟摇杆，在右侧新增 4 个主角技能意图入口；入口只发布统一 `hero-skill-intent`，不在 UI 内复制或伪造主动技能状态。
- 防线状态移到顶部关卡 HUD 下方，避免遮挡摇杆、技能与人物卡。
- 设计与实现均以 Cocos Creator 3.8.8 为唯一兼容基线。



## v0.5.0 赤金炎龙 Boss 系统

- 在现有第 40 波关底流程中正式接入赤金炎龙，保持 `ChapterWaveController -> EnemyController` 的唯一波次与敌人运行时链路。
- 新增数据驱动 `BossCatalog` 与受控的 `BossRuntimeController`：赤金炎龙拥有灼翼、熔甲、焚城三个生命阶段，以及龙炎吐息、焰尾横扫、陨石火雨三种带范围预警的技能。
- 新增战斗内 Boss 血条与阶段显示；Boss 仍复用 Hero 优先、随后“雕像 -> 城墙 -> 公主”的目标规则，并继续走统一经验、掉落、奖励和通关流程。
- 根据用户提供设计板制作并接入 768 × 603 透明 RGBA 战斗精灵；接入用户提供龙吼 MP3。WAV 未进入运行资源，音频仍由唯一 `AudioManager` 和唯一 `assets/resources/audio/` 目录管理。
- 明确 Cocos Creator 3.8.8 为唯一兼容基线；未引入其它 Creator 版本 API。

## v0.4.20 长屏设备战场顶部黑边修复

- 修复 iPhone 12 等长屏比例预览/设备上，主角向上推进后战场顶部露出 Camera 黑色清屏区域的问题。
- 根因：`BattleCameraController` 的地图边界 clamp 仍固定按 720×1280 的半视口 `360/640` 计算，而 Cocos 在更高纵横比设备上会扩大实际逻辑可见高度，导致世界层被允许向上移动过量。
- 镜头边界现在读取 Cocos `view.getVisibleSize()` 的实时可见逻辑尺寸，横向/纵向均按实际设备比例计算；720×1280 继续保持原手感。
- 增加 6 逻辑像素地图边缘安全覆盖，避免缩放/浮点舍入在边界位置露出 1px 清屏色。
- 每 0.5 秒轻量刷新一次可见视口尺寸，兼容编辑器预览窗口/设备比例变化，不在每帧产生额外尺寸查询。
- 本版本只修改战斗镜头边界计算，不改变地图玩法边界、角色移动范围、刷怪、英雄 AI、暂停或音频逻辑。
