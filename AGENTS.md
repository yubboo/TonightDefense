...
- `MainMenuBottomNav` 是大厅唯一底部主导航，固定包含：商店、英雄、战斗、仓库、升级；禁止为商店或其它页面再创建第二套底栏。
- 底部主导航在 `MainMenu.scene` 的五个主 Page 中必须常驻可见；切入商店、英雄、战斗、仓库、升级任一页时都不得隐藏或销毁。
- 五个入口分别切换独立 Page；页面只管理自身内容和局部子标签，不得接管、复制或覆盖主导航状态。
- 主导航选中态与 Page 切换统一由 `MainMenuView` 管理；各 Page 只能发出切页意图，不得私自维护另一份当前主页面状态。
- 页面布局必须为底栏预留安全高度；Page 专属背景和 ScrollView 必须精确结束在底栏顶边，既不得覆盖底栏，也不得留下透出其它页面背景的水平缝隙。购买按钮、页脚和交互热区不得延伸到底栏下方。
- 720 设计宽下，重要正文与交互文字优先不低于 16 px，辅助说明原则上不低于 14 px；确需更小字号时必须在目标设备预览中确认仍可读。
